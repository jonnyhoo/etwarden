...
use records::{parse_question, walk_a_aaaa_records};
pub use types::{DnsInfo, DnsQueryType};
...
pub fn analyze_dns(payload: &[u8]) -> Option<DnsInfo> {
    ...
    let qdcount = usize::from(u16::from_be_bytes([payload[4], payload[5]]));
    let ancount = usize::from(u16::from_be_bytes([payload[6], payload[7]]));
